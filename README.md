...
- `app`: 注入されたHTTP clientを使う日・週・月・習慣管理・バックアップ画面。基盤やCloudflareへ直接依存しない
- `browser`: 入口で使う製品名・説明
- `contracts`: 習慣・記録・日/週/月集計とDaymark backup schemaの入力・応答
- `server`: 日付、達成判定、設定履歴、集計、非破壊backup mergeを扱うdomain serviceとrepository interface
